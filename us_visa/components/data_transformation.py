...
class DataTransformation:
    ...
    def get_data_transformer_object(self) -> Pipeline:
        ...
        try:
            ...
            logging.info(
                "Initialized StandardScaler, OneHotEncoder, OrdinalEncoder")
    ...
    def initiate_data_transformation(self, ) -> DataTransformationArtifact:
        ...
        try:
            if self.data_validation_artifact.validation_status:
                ...
                logging.info(
                    "Got train features and test features of Training dataset")

                input_feature_train_df['company_age'] = CURRENT_YEAR - \
                    input_feature_train_df['yr_of_estab']

                logging.info(
                    "Added company_age column to the Training dataset")

                drop_cols = self._schema_config['drop_columns']

                logging.info(
                    "drop the columns in drop_cols of Training dataset")
                ...
                train_arr = np.c_[
                    input_feature_train_final, np.array(
                        target_feature_train_final)
                ]

                test_arr = np.c_[
                    input_feature_test_final, np.array(
